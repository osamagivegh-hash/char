...
export default function VisionPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!token) return;
      const res = await fetch(`${API}/dashboard/vision`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!token) return;
    await fetch(`${API}/dashboard/vision`, {
      method: "PUT",
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <h1 className="text-2xl font-bold text-slate-800">الرؤية</h1>
