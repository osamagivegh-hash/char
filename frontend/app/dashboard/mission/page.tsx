...
export default function MissionPage() {
  ...
  const [form, setForm] = useState<VisionMission>({ title: "", description: "" });
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!token) return;
      const res = await fetch(`${API}/dashboard/mission`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!token) return;
    await fetch(`${API}/dashboard/mission`, {
      method: "PUT",
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <h1 className="text-2xl font-bold text-slate-800">الرسالة</h1>
